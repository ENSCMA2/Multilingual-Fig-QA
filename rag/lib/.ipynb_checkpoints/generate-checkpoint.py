...
def extract_answer(llm_output: str):
    # some cleaning
...
class TogetherGeneratorBase(GeneratorBase):
    ...
    def __call__(self, prompt: str | ChatPromptValue, max_tokens=128, temperature=0.7, top_k=1) -> str:
        llm = Together(
            ...
            top_k=top_k,
            together_api_key="a32b0a749ba61f97b41fa26dcb4a7eb418fe7a087abb6de023711170df146f56"
        )
    ...
    def no_context_answer(self, question: str) -> tuple[str, str]:
        prompt = mk_no_context_qa_prompt(question, self.prompt_style)
        output: str = self(prompt, max_tokens=64, temperature=0.7, top_k=1)
        return extract_answer(output), prompt

    def answer_with_context(self, question: str, documents: list[Document]) -> tuple[str, str]:
